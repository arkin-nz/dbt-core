...
impl Auth for SparkAuth {
    ...
    fn configure(&self, config: &AdapterConfig) -> Result<AuthOutcome, AuthError> {
        // `Backend::Spark` serves both the `spark` and `fabricspark` adapters.
        // Fabric Lakehouse profiles are identified by their required GUID
        // fields and configured by the platform-specialized `fabric` module.
